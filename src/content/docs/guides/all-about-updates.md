...
We can use the [`debug_query`](https://docs.diesel.rs/2.3.x/diesel/fn.debug_query.html) function to inspect the generated SQL.
...
you can pass to `update` is anything which implements [the `Identifiable` trait](https://docs.diesel.rs/2.3.x/diesel/associations/trait.Identifiable.html).
`Identifiable` gets implemented by putting [`#[derive(Identifiable)]`](https://docs.diesel.rs/2.3.x/diesel/prelude/derive.Identifiable.html) on a struct.
...
If we look at the signature of [`.set`](https://docs.diesel.rs/2.3.x/diesel/query_builder/struct.UpdateStatement.html#method.set), you'll notice that the constraint is for a trait called
[`AsChangeset`](https://docs.diesel.rs/2.3.x/diesel/query_builder/trait.AsChangeset.html). This is another trait that `diesel` can derive for us. We can add
[`#[derive(AsChangeset)]`](https://docs.diesel.rs/2.3.x/diesel/prelude/derive.AsChangeset.html) to our `Post` struct, which will let us pass a `&Post` to `set`.
...
as well. See the [upsert docs](https://docs.diesel.rs/2.3.x/diesel/upsert/index.html) for more details.
...
The simplest method for running your query is [`execute`](https://docs.diesel.rs/2.3.x/diesel/query_dsl/trait.RunQueryDsl.html#tymethod.execute). This method will run your query,
...
we need to use [`get_result`](https://docs.diesel.rs/2.3.x/diesel/query_dsl/trait.RunQueryDsl.html#method.get_result) 
...
`#[derive(Queryable)]`). You should use 
[`get_results`](https://docs.diesel.rs/2.3.x/diesel/query_dsl/trait.RunQueryDsl.html#method.get_results) when you are expecting more than one record back.
If you are only expecting a single record, you can call 
[`get_result`](https://docs.diesel.rs/2.3.x/diesel/query_dsl/trait.RunQueryDsl.html#method.get_result) instead.
...
is considered an error condition by default.
...
you will be able to use the [`save_changes`](https://docs.diesel.rs/2.3.x/diesel/query_dsl/trait.SaveChangesDsl.html#method.save_changes) 
method. Unlike the other methods mentioned in this guide,
you do not explicitly build a query when using [`save_changes`](https://docs.diesel.rs/2.3.x/diesel/query_dsl/trait.SaveChangesDsl.html#method.save_changes).
...
All of the code for this guide can be found in executable form in 
